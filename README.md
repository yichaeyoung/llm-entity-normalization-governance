...
> **이미지 삽입 위치:** 프로젝트 전체 폴더 구조와  
> `Data Preprocessing → Search Family Relationship → Summarize & Making Results` 흐름을 보여주는 **Project File Structure 이미지**가 들어가면 됩니다.
...
> **이미지 삽입 위치:** 환경 변수와 입력 CSV를 불러온 뒤, 각 기업을 순회하면서  
> DuckDuckGo 검색, 기업 홈페이지 요약, LLM 질의, 응답 검증 및 CSV 저장 과정을 보여주는  
> **Company Info Extraction Pipeline Flow Chart 이미지**가 들어가면 됩니다.
